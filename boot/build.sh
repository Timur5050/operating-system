...
gcc -std=c99 -mcmodel=large -ffreestanding -fno-stack-protector -mno-red-zone \
    -c "$SCRIPT_DIR/../kernel/src/syscalls/syscall.c" -o "$SCRIPT_DIR/../build/kernel/syscall.o"
ld -nostdlib -T "$SCRIPT_DIR/../kernel/src/link.lds" -o "$SCRIPT_DIR/../build/kernel/kernel.elf" \
    ...
    "$SCRIPT_DIR/../build/kernel/process.o" "$SCRIPT_DIR/../build/kernel/syscall.o"
...
dd if="$SCRIPT_DIR/../build/boot/user.bin" of="$SCRIPT_DIR/../build/boot.img" bs=512 count=10 seek=106 conv=notrunc
